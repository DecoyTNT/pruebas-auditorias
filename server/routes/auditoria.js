...
app.put('/auditoria/:id', (req, res) => {
    ...
    Auditoria.findByIdAndUpdate(id, { $set: { normas: body.normas, grupoAuditor: body.grupoAuditor, auditados: body.auditados } }, { new: true, runValidators: true, context: 'query' }, (err, auditoriaDB) => {
        if (err) {
            return res.status(500).json({
                ok: false,
                err
